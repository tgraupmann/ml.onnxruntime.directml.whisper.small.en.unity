# Whisper Small EN (OnnxRuntime/DirectML) for Unity

https://assetstore.unity.com/packages/slug/352750

OpenAI Whisper is an automatic speech recognition (ASR) model trained on diverse audio; it transcribes speech to text for English. This package bundles the small English variant for offline transcription on Windows.
...
This is a Windows x64 package that embeds the OpenAI Whisper small English model inside a single native DLL, accelerated by ONNX Runtime with DirectML. Supports the Unity Editor and Windows Standalone 64-bit builds. Setup is turnkey and fully offline: no downloads, no updates—unpack the DLL, open the example scenes directly from the package, and choose your audio source (clip or microphone) for transcription or dictation.
...
## What’s inside
- Native plugin built on **ONNX Runtime + DirectML**; the full Whisper small EN model is contained in one DLL (no external model downloads, works fully offline).
...
## Package contents
- **Plugins/Windows/x64**: native DirectML/ONNXRuntime DLL with the embedded Whisper small EN model (unpacked via menu).
...
## Quick setup
1) **Unpack the DLL:** `Assets → WhisperSmallEn → Unpack WhisperSmallEn DLLs` (combines the split archive into a single native DLL in `Plugins/Windows/x64`). The native DLL is shipped split so each file stays under common version-control size limits (often ~100 MB). If your repo enforces that limit, you can add the unpacked DLL to `.gitignore` and rerun **Unpack WhisperSmallEn DLLs** after pulling or on machines that need to rebuild the extracted binary.
2) **Unpack test audio:** `Assets → WhisperSmallEn → Unpack WhisperSmallEn Example Audio` also drops the sample `WhisperTest.wav` into `Assets/StreamingAssets/Audio` for the clip-based example.
...
## Tech stack
- **Model:** OpenAI Whisper (small.en) converted to ONNX. See [available models](https://github.com/openai/whisper/tree/main?tab=readme-ov-file#available-models-and-languages).
...
## Minimal API
```csharp
var whisperSmallEn = new DllWhisperSmallEn();        // create the client (loads the native DLL/model)

// pcm: float[] mono, 16 kHz samples
var transcript = whisperSmallEn.Decode(pcm);         // decode transcript
```
...
### Progress
- `DllWhisperSmallEn.SetProgress(DllWhisperSmallEn.ProgressCallback callback)`
...
- `DllWhisperSmallEn.ClearProgress()`
...
### Cancellation
- `DllWhisperSmallEn.Abort()`
	- Requests cooperative cancellation of the currently-running native operation (for example, an in-flight `Decode`).
	- This is a global/native cancellation flag; it is not tied to a specific `DllWhisperSmallEn` instance.
